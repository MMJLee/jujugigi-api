...
async def read(auth_info: str = Security(authorize_user, scopes=[f"{CRUDOperation.READ}:{ResourceType.USER_IMAGE}"]),
...
async def update(auth_info: str = Security(authorize_user, scopes=[f"{CRUDOperation.UPDATE}:{ResourceType.USER_IMAGE}"]),
    ...
    user_email = auth_info
    added = await user_image_logic.update(user_image=user_image, user_email=user_email)
    return AddResponse(added=added)
