...
class ImageData:
    ...
    #     return Image(**mapped_dict)

    # def _map_model_to_dict(self, image: Image) -> Dict[str, Any]:
    #     mapped_dict = map_dictionary(
    ...
    async def create(self, image: ImageCreate) -> int:
        mapped_dict = self._map_model_to_dict(image=image)
        fields_stmt, values_stmt = build_insert_stmts(mapped_dict=mapped_dict)
        return await self._db.fetch_val(
            query=f"WITH create_image AS (INSERT INTO batch({fields_stmt}) VALUES({values_stmt}) RETURNING *) SELECT COUNT(*) as created FROM create_image;",
    ...
    async def update(self, id: int, image: Image) -> int:
        ...
        return await self._db.fetch_val(
            query=f"WITH update_image as (UPDATE image SET {update_stmt} WHERE id = :id RETURNING *) SELECT COUNT(*) as updated from update_image",
            values=filtered_dict, column='updated'
        )

    async def delete(self, id: int) -> int:
        return await self._db.fetch_val(
            query=f"WITH delete_image as (DELETE FROM image WHERE id = :id RETURNING *) SELECT count(*) as deleted from delete_image;",
